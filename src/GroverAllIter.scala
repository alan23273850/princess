...
class Class (val Q: Integer) extends App {
  ...
  SimpleAPI.withProver(enableAssert = debug) { p => // , logging = Set(Param.LOG_TASKS
    ...
    scope {
        ...
        !! (states.head  === sto(arrayN.const(complex(al, 0, 0, 0, 0)),
                                index ++ List(complex(ah, 0, 0, 0, 0)) : _*))
        !! (states.last  === sto(arrayN.const(complex(aL, 0, 0, 0, 2*Q)),
...
object GroverAllIter {
    def main(args: Array[String]): Unit = {
        new Class(args(0).toInt).main(args)
    }
}