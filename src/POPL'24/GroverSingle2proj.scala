...
class GroverSingle2projClass(val n: Int) extends Quantum(3) {
  ...
  SimpleAPI.withProver(enableAssert = debug, otherSettings = settings) { p => import p._
    ...
    scope {
        ...
        //////////////////////////
        ?? (proj(states.last, 2 -> False) === con(bools(2), complex(0, 0, 0, 0, countH))
          & proj(states.last, 2 -> True) === CartTheory.extTheories(bools(2)).store(List(con(bools(2), complex(0, 0, 0, 0, countH))) ++ index ++ List(complex(-1<<(countH/2), 0, 0, 0, countH)) : _*))
        println(???) // valid
        // println(countGate)
...
object GroverSingle2proj {
    def main(args: Array[String]): Unit = {
        new GroverSingle2projClass(2).main(args)
    }
}