...
class GroverSingle2selectClass(val n: Int) extends Quantum(3) {
  ...
  SimpleAPI.withProver(enableAssert = debug, otherSettings = settings) { p => import p._
    ...
    scope {
        ...
        !! (index =/= index2)
        //////////////////////////
        ?? (IFunApp(CartTheory.extTheories(bools(2)).select, List(proj(states.last, 2 -> False)) ++ createConstants(2, Sort.Bool)) === complex(0, 0, 0, 0, countH)
          & IFunApp(CartTheory.extTheories(bools(2)).select, List(proj(states.last, 2 -> True)) ++ index) === complex(-1<<(countH/2), 0, 0, 0, countH)
          & IFunApp(CartTheory.extTheories(bools(2)).select, List(proj(states.last, 2 -> True)) ++ index2) === complex(0, 0, 0, 0, countH))
        println(???) // valid
        // println(countGate)
...
object GroverSingle2select {
    def main(args: Array[String]): Unit = {
        new GroverSingle2selectClass(2).main(args)
    }
}