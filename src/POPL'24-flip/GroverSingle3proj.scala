...
class GroverSingle3projClass(val n: Int) extends Quantum(2*n) {
  import CartTheory.{arraySto, proj, con}
  SimpleAPI.withProver(enableAssert = debug, otherSettings = settings) { p => import p._
    ...
    scope {
        ...
        //////////////////////////
        ?? (proj(states.last, 5 -> False) === con(bools(5), complex(0, 0, 0, 0, 17))
          & proj(states.last, 5 -> True, 4 -> True) === con(bools(4), complex(0, 0, 0, 0, 17))
          & proj(states.last, 5 -> True, 4 -> False, 2 -> True) === con(bools(3), complex(0, 0, 0, 0, 17))
          & proj(states.last, 5 -> True, 4 -> False, 2 -> False) === CartTheory.extTheories(bools(3)).store(List(con(bools(3), complex(-32, 0, 0, 0, 17))) ++ index ++ List(complex(352, 0, 0, 0, 17)) : _*))
        println(???) // valid
...
object GroverSingle3proj {
    def main(args: Array[String]): Unit = {
        new GroverSingle3projClass(3).main(args)
    }
}